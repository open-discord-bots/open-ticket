...
```jsonc
{
    ...
    "pdfTranscriptStyle": {
        ...
        "branding": {
            ...
            "subtitle": "Generated by Open Ticket",
            "logoUrl": "",
            "logoPath": "",
            "showServerIcon": true,
            "showBotBranding": true,
            "footerText": "Open Ticket Transcript",
            "watermarkText": "",
            "showWatermark": false
        },
        "colors": {
            ...
            "warning": "#FEE75C",
            "danger": "#ED4245",
            "attachmentBackground": "#F8F9FA",
            "attachmentBorder": "#DADDE1"
        },
        "layout": {
            "pageSize": "A4",
            "margins": {
        ...
        "attachments": {
            ...
            "sendArchiveFilesSeparately": true,
            "archiveExtensions": [".tar.gz", ".tgz", ".zip", ".tar", ".rar", ".7z", ".gz", ".bz2", ".xz"],
            "archiveContentTypes": ["application/zip", "application/x-zip-compressed", "application/x-tar", "application/gzip", "application/x-gzip", "application/x-7z-compressed", "application/vnd.rar", "application/x-rar-compressed", "application/x-bzip2", "application/x-xz"],
            "maxSeparateFileSizeMB": 25,
            "maxTotalSeparateFilesSizeMB": 50,
            "fallbackToAttachmentLinks": true
        }
    }
}
```

PDF transcripts include ticket metadata, ordered messages, embed/sticker summaries, and attachment metadata. Image previews are downloaded only from Discord attachment URLs and are skipped safely when a file is too large, unavailable, or unsupported. Archive attachments such as ZIP, TAR, RAR, and 7Z files remain listed in the PDF and can also be forwarded as separate Discord attachments with the transcript, subject to the configured per-file and total size limits. If an archive cannot be forwarded, the PDF keeps its metadata and URL. If PDF generation fails, Open Ticket logs the error and falls back to HTML when possible.
